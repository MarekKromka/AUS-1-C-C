...
namespace structures 
{
	...
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		if (this != &other) {
			...
			{
				throw std::invalid_argument("Sizes of arrays are different.");
			}
	...
	T& Array<T>::operator[](const int index)
	{
		...
		byte * byteAdr = vector_->getBytePointer(vectorIndex);
		T* tAdr = reinterpret_cast<T*>(byteAdr);
		return *tAdr;
	}
	...
	{
		return size_ == other.size_ && *vector == *other.vector_;
	}
	...
	void Array<T>::copy(const Array<T>& src, const int srcStartIndex, Array<T>& dest, const int destStartIndex, const int length)
	{
		...
		structures::Vector::copy(*src.vector_, srcStartIndex * sizeof(T), *dest.vector_, destStartIndex * sizeof(T), length * sizeof(T));

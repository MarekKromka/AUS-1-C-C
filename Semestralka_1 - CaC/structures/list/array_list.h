...
namespace structures
{
	...
	{
		DSRoutines::rangeCheckExcept(index, size_ + 1, "Invalid index. removeAtAR");
		T data = (*array_)[index];
		(*array_)[index] = 0;
	...
	{
		for (size_t i = 0; i < static_cast<int>(size_); i++)
		{
			if ((*array_)[i] == data)
				return i;
		}
	...
	inline void ArrayList<T>::enlarge()
	{
		if (array_->size() == size_) {
			Array<T> * pom = new Array<T>(array_->size() > 0 ? size_ * 2 : 4);
			array_->copy(*array_, 0, *pom, 0, array_->size());
			delete array_;
			array_ = pom;
		}

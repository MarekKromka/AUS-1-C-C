...
namespace structures
{
	...
	inline bool LinkedList<T>::tryRemove(const T & data)
	{
		...
		LinkedListItem<T> * del = cur->getNext();
		for (size_t i = 0; i < size_; i++)
		{
			if (del->accessData() == data) {
	...
	inline T LinkedList<T>::removeAt(const int index)
	{
		...
		LinkedListItem<T> * prev = first_;
		if (index == 0) { // ak sa snazis zmazat prvy prvok
			LinkedListItem<T> * del = first_;
			first_ = first_->getNext();
			T pom = prev->accessData();
			delete del;
			size_--;
			return pom;
		}
		...
		LinkedListItem<T> * del = prev->getNext();
		prev->setNext(del->getNext());
		T pom = del->accessData();
		delete del;
		size_--;
		return pom;
	...
	inline int LinkedList<T>::getIndexOf(const T & data)
	{
		...
		for (size_t i = 0; i < size_; i++)
		{
			if (cur->accessData() == data) return i;
			cur = cur->getNext();
		}
	}
